# funcs/dashboard_vencimientos.py
...
class VencimientosFrame(ctk.CTkFrame):
    ...
    def _build(self):
        ctk.CTkLabel(self, text="Vencimientos", font=ctk.CTkFont(size=18, weight="bold")).pack(pady=(8,12))
    ...
    def update_contents(self):
        ...
        cur = conn.cursor()
        cur.execute("SELECT v.cdb, p.nombre, v.cantidad, v.fecha_vencimiento FROM vencimientos v LEFT JOIN producto p ON v.cdb=p.cdb ORDER BY v.fecha_vencimiento")
        rows = cur.fetchall()
        conn.close()
        for r in rows:
            cdb, nombre, cantidad, fv = r
            lbl = ctk.CTkLabel(self.list_frame, text=f"{cdb} - {nombre or 'N/D'} - Cant: {cantidad} - Vto: {fv}")
    ...
    def _add(self):
        ...
        try:
            ...
            conn.commit()
            conn.close()
            mb.showinfo("OK", "Vencimiento agregado")
