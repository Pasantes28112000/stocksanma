...
class CompraFrame(ctk.CTkFrame):
    ...
    def _render_cart(self):
        ...
        for idx, item in enumerate(self.cart):
            # usar formatting.format_amount para precio unitario (respeta símbolo y formato)
            unit = formatting.format_amount(item['precio'])
            txt = f"{item['cdb']} - {item['nombre']} x {item['cantidad']} @ {unit}"
        ...
        # formatear total con preferences
        self.total_label.configure(text=f"Total: {formatting.format_amount(self.total)}")
    ...
    def _confirmar(self):
        if not self.cart:
            ...
            return
        try:
            self._register_purchase(self.cart)
    ...
    def _register_purchase(self, items):
        """
        Registrar una compra completa.
        items: lista de dicts: [{"cdb": <int>, "cantidad": <int>, "precio": <float>, "vencimiento": <str>|None}, ...]
        Inserta fila en compra, luego inserta en compra_detalle usando columna 'precio_compra'
        y actualiza el stock en producto.cantidad y producto.precio.
        """
        conn = get_connection(self.db)
        cur = conn.cursor()
        try:
            # insertar compra (fecha explícita)
            ...
            for it in items:
                ...
                res = cur.fetchone()
                if not res:
                    raise ValueError(f"Producto inexistente: {it.get('nombre', cdb)}")
                nueva = (res[0] or 0) + cant

                # actualizar stock y precio del producto -> usar columna 'precio'
                cur.execute("UPDATE producto SET cantidad=?, precio=? WHERE cdb=?", (nueva, precio, cdb))

                # insertar detalle con la columna precio_compra
                cur.execute(
                    "INSERT INTO compra_detalle (compra, cdb, cantidad, precio_compra) VALUES (?, ?, ?, ?)",
                    (cid, cdb, cant, precio)
                )

                # actualizar caja/dinero (resta por compra)
                cur.execute("UPDATE dinero SET total = total - ? WHERE id=1", (precio * cant,))

                # insertar vencimiento si corresponde (campo fecha_vencimiento)
                if it.get('vencimiento'):
                    try:
                        cur.execute("INSERT INTO vencimientos (cdb, cantidad, fecha_vencimiento) VALUES (?, ?, ?)",
                                    (cdb, cant, it['vencimiento']))
                    except Exception:
                        # si la tabla/columnas no existen, ignorar ese insert
                        pass
...
class CompraPicker(ctk.CTkToplevel):
    ...
    def _buscar(self):
        ...
        for p in products:
            ...
            # formatear precio del producto usando formato global
            txt = f"{cdb} - {nombre} (Stock: {cantidad}) - {formatting.format_amount(precio)}"
